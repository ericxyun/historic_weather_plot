...
#%%
data_year = {x: data[data['year'] == x] for x in data['year'].unique()}

#%%

matplotlib.style.use('seaborn')
plt.figure(figsize=(20,5))
x = data_year[2017]['Date']
y = data_year[2017]['min_daily']
year_dict = find_min_max(2017)
plt.axvline(x=x_min, color='black', ymin=.25, ymax=.75)
plt.axvline(x=x_max, color='black')
plt.scatter(x_min, 50, s=150, facecolors='none', edgecolors='r')
...
ax.annotate('local max', xy=(2, 1), xytext=(5, 1.5),
